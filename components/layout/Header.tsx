...
export function Header() {
  ...
  const [showSalawatBanner, setShowSalawatBanner] = useState(true);
  ...
  useEffect(() => {
    ...
    const dismissed = sessionStorage.getItem("salawat-banner-dismissed");
    if (dismissed) setShowSalawatBanner(false);
  ...
  const dismissBanner = () => {
    setShowSalawatBanner(false);
    sessionStorage.setItem("salawat-banner-dismissed", "true");
  };
  ...
  return (
    <>
      {/* ============================================= */}
      {/* Salawat Announcement Banner */}
      {/* ============================================= */}
      {showSalawatBanner && (
        <div
          className="relative z-50 flex items-center justify-center gap-3 px-4 py-2 text-sm font-semibold text-white shadow-sm"
          style={{
            background: "linear-gradient(90deg, #064e3b, #0f766e, #064e3b)",
            backgroundSize: "200% 100%",
            animation: "salawat-slide 4s linear infinite",
          }}
        >
          <style>{`
            @keyframes salawat-slide {
          ...
          <span className="text-center font-bold tracking-wide drop-shadow-md text-[13px] md:text-[14px]">
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-6">
              ...
              <nav className="hidden xl:flex items-center gap-8">
                ...
                <Link href="/series?type=manga" className="text-sm font-semibold hover:text-primary-500 transition-colors" style={{ color: "var(--text-secondary)" }}>
                  مانجا
                </Link>
                <Link href="/series?type=manhwa" className="text-sm font-semibold hover:text-primary-500 transition-colors" style={{ color: "var(--text-secondary)" }}>
                  مانهوا
                </Link>
              </nav>
            </div>
            ...
            <form onSubmit={handleSearch} className="hidden md:flex flex-1 max-w-lg mx-6 relative group">
               ...
               <input
                 ...
                 placeholder="ابحث عن مانهوا أو مانجا..."
        ...
        {isMenuOpen && (
          <div className="md:hidden glass-panel border-x-0 border-t" style={{ animation: "slide-up 0.3s ease-out" }}>
            <div className="px-4 pt-2 pb-6 space-y-1 shadow-2xl">
              ...
              <Link href="/series?type=manga" className="block px-4 py-3 rounded-xl text-base font-semibold hover:bg-[var(--card-hover)]" style={{ color: "var(--text-primary)" }}>
                مانجا
              </Link>
              <Link href="/series?type=manhwa" className="block px-4 py-3 rounded-xl text-base font-semibold hover:bg-[var(--card-hover)]" style={{ color: "var(--text-primary)" }}>
                مانهوا
              </Link>
            </div>
          </div>
