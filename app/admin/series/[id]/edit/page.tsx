...
export default function EditSeriesPage() {
    ...
    const fetchSeries = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          // Map genres
          if (data.genres) {
            const slugs = data.genres.map((g: any) => g.genre?.slug).filter(Boolean);
            setSelectedGenres(slugs);
          }
        }
      } catch (err: any) {
        console.error(err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let cover_image_url = coverPreview && !coverFile ? coverPreview : null;

      // 1. Upload Cover Image to Vultr Object Storage if completely changed
      if (coverFile) {
        ...
        // user removed the cover completely
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs mt-2 leading-relaxed" style={{ color: "var(--text-muted)" }}>
                عند تحديث الصورة، ستُرفع مباشرة إلى{" "}
